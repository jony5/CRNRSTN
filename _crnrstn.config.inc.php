...
if(!$oCRNRSTN_ENV->isConfigured($oCRNRSTN)){
	...
	* Note that the available cipher methods can differ between your dev server and your production server! They will depend on the installation 
	and compilation options used for OpenSSL in your machine(s). Run crnrstn_config_debug.php to see SERVER's available OpenSSL ciphers & hash algorithms.
	...
	$oCRNRSTN->initSessionEncryption([environment-key] -> Specify one of your previously defined addEnvironment() environment keys , 
									   [openssl-encryption-cipher] -> For a list of recommended (and available) openssl cipher methods...run $oCRNRSTN_ENV->openssl_get_cipher_methods();
	...
	*/
	...
	$oCRNRSTN->initTunnelEncryption('LOCALHOST_PC', 'AES-256-CTR', 'this-Is-the-encryption-key', OPENSSL_RAW_DATA, 'ripemd256');
	...
	#$CRNRSTN_minServerMatchCnt = 3;		// HOW MANY SERVER KEYS WILL YOU DEFINE (BELOW) FOR EACH ENVIRONMENT TO SUPPORT CRNRSTN ENVIRONMENTAL DETECTION? TEST *ALL* FORMATS OF WEB SITE DOMAIN.
	...
	// ** BE SURE TO TEST YOUR CRNRSTN CONFIGURATION FOR ALL POSSIBLE 'SERVER_NAME' VARIATIONS...E.G. WWW AND NON-WWW WHERE APPLICABLE **
	...
	$oCRNRSTN->defineEnvResource('LOCALHOST_MAC', 'WSDL_URI', 'http://172.16.225.128/services/soa/crnrstn/1.0.0/wsdl/index.php?wsdl');	# KEY "WSDL_URI" (TO YOUR ENDPOINT) REQUIRED BY CRNRSTN SOAP CLIENT CONNECTION MANAGER IF USING CRNRSTN SOAP CLIENT MANAGER
	...
	$oCRNRSTN->defineEnvResource('LOCALHOST_PC', 'SESSION_EXPIRE', 'INTERVAL 30 MINUTE');
