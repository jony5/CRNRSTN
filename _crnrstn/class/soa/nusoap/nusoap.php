...
class wsdl extends nusoap_base {
    ...
    */
    function webDescription(){
		...
		<style type="text/css">
			...
			background-color: #ccccff; width: 24%; margin-left: 20px; margin-top: 20px; }
		    .title {
		...
		<div class=content>
			...
			<div class=nav>
				...
				Click<br>on an operation name to view it&apos;s details.</p>
				<ul>';
	...
	*/
	...
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
		$this->debug("in serializeRPCParametters: operation=$operation, direction=$direction, XMLSchemaVersion=$this->XMLSchemaVersion, bindingType=$bindingType");
		...
		$this->debug('in serializeRPC!Parameters: opData:');
		$this->appendDebug($this->varDump($opData));
		$this->debug('J5, WE ARE HERE....!');
			...
			if (is_array($parameters)) {
				...
				foreach ($parts as $name => $type) {
					...
					if ($parametersArrayType == 'arraySimple') {
						...
						$this->debug('J5, WE HAVE DROPPED PARAMS HERE(10)....');
	...
	{
		$this->debug('J5, WE START TO DROP PARAMS HERE(11). VALUE :: '.$value);
		...
		$xml = '';
		$this->debug('J5, WE START TO DROP PARAMS HERE(55). VALUE :: '.$value);
		if (strpos($type, ':')) {
			$uqType = substr($type, strrpos($type, ':') + 1);
			$ns = substr($type, 0, strrpos($type, ':'));
		...
		$phpType = $typeDef['phpType'];
		$this->debug('J5, WE ARE HERE..88. VALUE :: '.sizeof($value));
		...
		if ($phpType == 'struct') {
			...
			if ($use == 'literal') {
				...
				$this->debug('J5, WE ARE HERE....100');
				$xml = "<$name$elementNS xsi:type=\"" . $this->getPrefixFromNamespace($ns) . ":$uqType\"$encodingStyle>$value</$name>";
			}
		}
		$this->debug('J5, WE ARE HERE..99..');
		$this->debug("in serializeType: returning: $xml");
		$this->debug('J5, WE HAVE DROPPED PARAMS HERE....?');
		return $xml;
	}
		...
		if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
			...
			foreach ($typeDef['elements'] as $eName => $attrs) {
				...
				if (isset($optionals)
					...
					// do nothing
					$this->debug('J5, WE HAVE DROPPED PARAMS HERE(400)....?');
