<?php
/* 
// J5
// Code is Poetry */

//
// INITIALIZE SECURITY PROTOCOLS FOR RESOURCE ACCESS [IPv4 and IPv6 (testing needed on v6)]
// GRANT EXCLUSIVE ACCESS TO THE FOLLOWING IPs. ALL OTHER IP NOT REPRESENTED HERE WILL BE 403'D.
#
# $this->oCRNRSTN_IPSECURITY_MGR->grantAccess([environment-key], [ip,ip-ip,ip]);
$this->oCRNRSTN_IPSECURITY_MGR->grantAccess('LOCALHOST_PC','127.0.0.1, 197.201.10.5');
$this->oCRNRSTN_IPSECURITY_MGR->grantAccess('LOCALHOST_MAC','172.16.110.*');
#$this->oCRNRSTN_IPSECURITY_MGR->grantAccess('STAGE','FE80::230:80FF:FEF3:4701');
#$this->oCRNRSTN_IPSECURITY_MGR->grantAccess('PREPROD','127.0.0.1, 127.*, 128.0.4.50-128.0.4.60, 129.*-130.50.*, 130.51.10.*');
#$this->oCRNRSTN_IPSECURITY_MGR->grantAccess('PRODUCTION','127.0.0.1, 130.*, 128.0.4.50-129.0.*, 129.*-130.50.10.10, 130.51.10.*');

?>